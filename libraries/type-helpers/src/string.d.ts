...
type _Join<T extends readonly Stringable[], D extends string> = T extends [] ? ''
...
type _ClearEmpties<T extends readonly Stringable[], Result extends readonly Stringable[]> = T extends
  [infer X extends Stringable, ...infer Rest extends readonly Stringable[]]
  ? _ClearEmpties<Rest, IfEmpty<X, Result, [...Result, X]>>
  : Result;
...
type _Split<S extends Stringable, D extends Stringable, Acc extends readonly string[]> = S extends
  `${infer T}${D}${infer U}` ? _Split<U, D, IfEmpty<T, Acc, [...Acc, T]>> //  [T, ..._Split<U, D>] :
  : IfEmpty<S, Acc, [...Acc, S]>;
