...
import { AnyOf, Fu, NoneOf, ReplaceEnding } from '../util';
import {ToInfinitive as IrregularVerbs} from './irregular-verbs';

export type ToInfinitive<T> =
    ...
    T extends `${string}${'h' | 'ion' | `n${AnyOf<'dt'>}` | `ai${Fu}` | `${AnyOf<'cs'>}t` | `pp` | `all` | `ss` | `tt` | `int` | `ail` | `ld` | `en` | `oo${Fu}` | `er` | `k` | `pp` | `w` | `ou${Fu}` | `rt` | `ght` | `rm`}ed` ? ReplaceEnding<T, 'ed', ''> :

    // (..[^aeiou])ed => '$1e'
    T extends `${string}${string}${NoneOf<'aeiou'>}ed` ? ReplaceEnding<T, 'ed', ''> :
