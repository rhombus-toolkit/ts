import { Not, NoneOf, ReplaceEnding, AnyOf } from '../util';
...
export type PluralizationRules<T extends string> =

    // //[/[^\u0000-\u007F]$/i, '$0'],
    // T extends `${infer X}${Not<infer Y, Fu>}` ? X :

    // [/([^aeiou]ese)$/i, '$1'],
    T extends `${string}${NoneOf<'aeiou'>}ese` ? T :
    ...
    T extends `${infer X}${`${NoneOf<'l'>}ias` | `${AnyOf<'aeiou'>}las` | `${AnyOf<'ejzr'>}as` | `${AnyOf<'iu'>}am`}` ? T :

    // [/(alumn|syllab|vir|radi|nucle|fung|cact|stimul|termin|bacill|foc|uter|loc|strat)(?:us|i)$/i, '$1i'],
    T extends `${string}${'alumn' | 'syllab' | 'vir' | 'radi' | 'nucle' | 'fung' | 'cact' | 'stimul' | 'termin' | 'bacill' | 'foc' | 'uter' | 'loc' | 'strat'}${'us' | 'i'}` ? ReplaceEnding<T, 'us'|'i', `i`> :
