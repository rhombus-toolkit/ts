...
export type Consonant = 'b' | 'c' | 'd' | 'f' | 'g' | 'h' | 'j' | 'k' | 'l' | 'm' | 'n' | 'p' | 'q' | 'r' | 's' | 't'
  | 'v' | 'w' | 'x' | 'y' | 'z';
...
export type LastChar<T extends string> = T extends `${infer Head}${infer Rest}` ? Rest extends '' ? T : LastChar<Rest>
  : never;
